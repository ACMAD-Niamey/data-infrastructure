...
| [Accessing data via STAC](stac-access.md) | Browsing/searching the STAC API, reading raster assets (s3:// hrefs via GDAL `/vsis3/`), public vs private bucket access. |
| [THREDDS download & ingestion](thredds-ingestion.md) | Automated GeoTIFF pull from ACMAD THREDDS (daily forecasts, monthly/seasonal observed products) into existing datasets/styles: workflow config, cadence, idempotency, backfill, and scheduling. |
